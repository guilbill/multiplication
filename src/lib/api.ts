import { initMultProgress, initConjProgress, initVerbConjProgress } from './spacedRepetition'
...
function parseSaved(raw: unknown): { mult: Record<string, number>; conj: Record<number, number>; verbConj: Record<number, number>; xp: number } {
  ...
  return { mult: (raw ?? {}) as Record<string, number>, conj: {}, verbConj: {}, xp: 0 }
}
...
export async function loadProgress(profile: string): Promise<{
  ...
  try {
    ...
    const { mult, conj, verbConj, xp } = parseSaved(raw ? JSON.parse(raw) : {})
    return {
...
export async function saveProgress(
  ...
  try {
    localStorage.setItem(PREFIX + profile, JSON.stringify({ mult: multProgress, conj: conjProgress, verbConj: verbConjProgress, xp }))
  } catch {
